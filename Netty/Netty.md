...
> 非阻塞模式存在的问题：单线程对线程的损耗太大，只有一个线程不断执行，并且如果没有连接请求的时候，这个线程仍然在不断地执行`while(true)`这个循环，造成了CPU资源的浪费
...
> 为什么`sc.register(worker.selector, SelectionKey.OP_READ, null);`一定要在`selector.select()`执行？
>
...
整个过程仅只发生了**一次用户态与内核态的切换**，数据拷贝了 2 次。所谓的【零拷贝】，并不是真正无拷贝，而是在**不会拷贝重复数据到 jvm 内存中**，零拷贝的优点有
...
从channel中的例子中也可以看出，Netty中可以才采用异步的方式进行数据的处理，那么，异步操作对数据处理操作的效率提升是哪方面的呢？
