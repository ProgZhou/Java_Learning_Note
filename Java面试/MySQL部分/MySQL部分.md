...
如果不使用索引下推，假设利用zipcode索引筛选出来了10000条数据，需要将这10000条数据全部回表进行剩余字段的比较，这样性能就不好；如果使用索引下推，利用联合索引中含有的lastname字段，在二级索引的时候就顺便去过滤掉lastname不包含'张'的数据，假设过来吧之后只剩500条数据了，那么只需要将这500条数据回表即可，不需要回表10000条数据，提高了效率
